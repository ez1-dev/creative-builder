-- Limpar widgets antigos do bloco único de gráficos
DELETE FROM public.dashboard_widgets
WHERE type = 'charts-row'
  AND dashboard_id IN (
    SELECT id FROM public.dashboards WHERE module = 'passagens-aereas'
  );

-- Reescreve a função para a nova lista canônica com 5 gráficos individuais
...
BEGIN
  ...
  FOR rec IN
    SELECT * FROM (VALUES
      ...
      ('tabela-registros'::text,      'Registros'::text,              6, '{"x":0,"y":27,"w":12,"h":10}'::jsonb)
...
-- Repovoa o dashboard padrão com a nova lista
