-- Remove versão antiga (integer, text) para evitar overload ambíguo
DROP FUNCTION IF EXISTS public.bi_dre_matriz_anual(integer, text);

-- Recria com assinatura (text, text) — compatível com supabase.rpc do frontend
...
BEGIN
  ...
  base AS (
    ...
    FROM meses m
    WHERE upper(unaccent(m.e_descricao)) LIKE 'RECEITA LIQUIDA%'
  )
  SELECT
    m.ordem,
...
GRANT EXECUTE ON FUNCTION public.bi_dre_matriz_anual(text, text) TO anon, authenticated, service_role;
