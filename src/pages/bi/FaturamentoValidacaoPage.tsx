...
export default function FaturamentoValidacaoPage() {
  ...
  const movColumns: Column<PorMovimentoRow>[] = useMemo(() => [
    { key: 'anomes_emissao', header: 'AnoMês', accessor: r => r.anomes_emissao ?? '-' },
    { key: 'cd_tp_movimento', header: 'Tp Mov', accessor: r => r.cd_tp_movimento ?? '-' },
    { key: 'cd_origem', header: 'Origem', accessor: r => r.cd_origem ?? '-' },
    { key: 'qtd_linhas', header: 'Linhas', accessor: r => formatNumber(num(r.qtd_linhas)), align: 'right' },
    { key: 'vl_bruto', header: 'VL Bruto', accessor: r => formatCurrency(num(r.vl_bruto)), align: 'right' },
    { key: 'vl_total', header: 'VL Total', accessor: r => formatCurrency(num(r.vl_total)), align: 'right' },
    { key: 'vl_devolucao', header: 'VL Devolução', accessor: r => formatCurrency(num(r.vl_devolucao)), align: 'right' },
    { key: 'vl_icms', header: 'VL ICMS', accessor: r => formatCurrency(num(r.vl_icms)), align: 'right' },
    { key: 'vl_pis', header: 'VL PIS', accessor: r => formatCurrency(num(r.vl_pis)), align: 'right' },
    { key: 'vl_cofins', header: 'VL COFINS', accessor: r => formatCurrency(num(r.vl_cofins)), align: 'right' },
  ], []);

  const tnsColumns: Column<PorTnsRow>[] = useMemo(() => [
    { key: 'cd_tns', header: 'TNS', accessor: r => r.cd_tns ?? '-' },
    { key: 'cd_natureza', header: 'Natureza', accessor: r => r.cd_natureza ?? '-' },
    { key: 'qtd_linhas', header: 'Linhas', accessor: r => formatNumber(num(r.qtd_linhas)), align: 'right' },
    { key: 'vl_total', header: 'VL Total', accessor: r => formatCurrency(num(r.vl_total)), align: 'right' },
    { key: 'vl_devolucao', header: 'VL Devolução', accessor: r => formatCurrency(num(r.vl_devolucao)), align: 'right' },
  ], []);
  ...
  const detColumns: Column<DetalheRow>[] = useMemo(() => [
    { key: 'cd_tp_movimento', header: 'Tp Mov', accessor: r => r.cd_tp_movimento ?? '-' },
    { key: 'cd_origem', header: 'Origem', accessor: r => r.cd_origem ?? '-' },
    { key: 'cd_empresa', header: 'Empresa', accessor: r => r.cd_empresa ?? '-' },
    { key: 'cd_filial', header: 'Filial', accessor: r => r.cd_filial ?? '-' },
    { key: 'cd_nf', header: 'NF', accessor: r => r.cd_nf ?? '-' },
    { key: 'cd_serie', header: 'Série', accessor: r => r.cd_serie ?? '-' },
    { key: 'dt_emissao', header: 'Dt Emissão', accessor: r => formatDateBR(r.dt_emissao) },
    { key: 'anomes_emissao', header: 'AnoMês', accessor: r => r.anomes_emissao ?? '-' },
    { key: 'cd_tns', header: 'TNS', accessor: r => r.cd_tns ?? '-' },
    { key: 'cd_cliente', header: 'Cliente', accessor: r => r.cd_cliente ?? '-' },
    { key: 'cd_centro_custos_3', header: 'CC3', accessor: r => r.cd_centro_custos_3 ?? '-' },
    { key: 'vl_bruto', header: 'VL Bruto', accessor: r => formatCurrency(num(r.vl_bruto)), align: 'right' },
    { key: 'vl_total', header: 'VL Total', accessor: r => formatCurrency(num(r.vl_total)), align: 'right' },
    { key: 'vl_devolucao', header: 'VL Devolução', accessor: r => formatCurrency(num(r.vl_devolucao)), align: 'right' },
    { key: 'created_at', header: 'Criado em', accessor: r => formatDateBR(r.created_at) },
  ], []);
