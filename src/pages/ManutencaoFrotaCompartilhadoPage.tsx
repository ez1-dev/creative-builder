...
import { deriveEffectiveToken } from '@/components/frota/FrotaShareLinksDialog';
...
export default function ManutencaoFrotaCompartilhadoPage() {
  ...
  const loadData = async (effectiveToken: string) => {
    ...
    const [{ data: rows, error }, { data: visuals }] = await Promise.all([
      supabase.rpc('get_frota_via_token', { _token: effectiveToken }),
      supabase.rpc('get_frota_share_link_visuals', { _token: effectiveToken }),
    ]);
    setSubmitting(false);
    if (error) {
    ...
    setData((rows as ManutencaoFrota[]) ?? []);
        ...
        <Card className="max-w-md w-full">
          <CardContent className="p-8 text-center space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              {state === 'expired'
                ? 'Este link expirou. Solicite um novo ao administrador.'
        ...
        <Card className="max-w-md w-full">
          <CardContent className="p-8 space-y-4">
            <div className="text-center space-y-2">
              ...
              <h1 className="text-lg font-bold">Manutenção de Frota</h1>
            ...
            <form onSubmit={(e) => { e.preventDefault(); handlePasswordSubmit(); }} className="space-y-3">
              <div>
                <Label htmlFor="senha-frota-compartilhado">Senha</Label>
                <Input id="senha-frota-compartilhado" name="senha-frota-compartilhado" type="password" value={password} onChange={(e) => setPassword(e.target.value)} autoFocus />
              </div>
