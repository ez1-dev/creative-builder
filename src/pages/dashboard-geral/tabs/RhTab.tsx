  ...
  return (
    <div className="space-y-4">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <LineChartCard title="Headcount — evolução" data={data.series.headcount} valueFormatter={formatNumber} color="hsl(var(--success))" height={260} />
        <BarChartCard title="Turnover mensal" data={data.series.turnover_mes} valueFormatter={(v) => formatPercent(v, 1)} color="hsl(var(--warning))" height={260} />
        <HorizontalBarChartCard title="Absenteísmo — motivos" data={data.breakdowns.absenteismo_motivo} valueFormatter={(v) => `${formatNumber(v)} dias`} color="hsl(var(--destructive))" height={280} />
        <HorizontalBarChartCard title="Colaboradores por setor" data={data.breakdowns.setor} valueFormatter={formatNumber} height={280} />
      </section>
    </div>
  );
}
