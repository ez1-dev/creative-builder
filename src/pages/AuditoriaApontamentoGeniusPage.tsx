...
import {
  ClipboardCheck, AlertTriangle, AlertCircle, Clock, UserCheck, ListChecks, FileQuestion, Timer,
} from 'lucide-react';
...
const columns: Column<any>[] = [
  ...
  { key: 'total_dia_operador', header: 'Total Dia Operador', align: 'right', render: (v) => formatNumber(v, 2) },
  {
    ...
    header: 'Status',
    render: (v: Status) => {
      const cfg = statusVariants[v] || statusVariants.OK;
    ...
    <div className="space-y-4 p-4">
      ...
      {atualizarKpisApontGenius && (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
          ...
          <KPICard title="Discrepâncias" value={formatNumber(atualizarKpisApontGenius.total_discrepancias, 0)} icon={<AlertCircle className="h-5 w-5" />} variant="destructive" index={1} />
          <KPICard title="Sem Início" value={formatNumber(atualizarKpisApontGenius.sem_inicio, 0)} icon={<FileQuestion className="h-5 w-5" />} variant="warning" index={2} />
          <KPICard title="Sem Fim" value={formatNumber(atualizarKpisApontGenius.sem_fim, 0)} icon={<FileQuestion className="h-5 w-5" />} variant="warning" index={3} />
          <KPICard title="Fim < Início" value={formatNumber(atualizarKpisApontGenius.fim_menor_inicio, 0)} icon={<Timer className="h-5 w-5" />} variant="destructive" index={4} />
          <KPICard title="Acima de 8h" value={formatNumber(atualizarKpisApontGenius.acima_8h, 0)} icon={<Clock className="h-5 w-5" />} variant="destructive" index={5} />
          <KPICard
