...
const columns: Column<any>[] = [
  ...
  { key: 'quantidade_patio', header: 'Qtd Pátio', align: 'right', render: (v) => formatNumber(v, 0) },
  { key: 'peso_kg', header: 'Peso (Kg)', align: 'right', render: (v) => formatNumber(v, 1) },
  { key: 'localizacao', header: 'Localização' },
  { key: 'dias_em_patio', header: 'Dias no Pátio', align: 'right' },
];
...
export default function SaldoPatioPage() {
  ...
  const search = useCallback(async (page = 1) => {
    ...
    setLoading(true);
    try {
      const result = await api.get<PatioResponse>('/api/producao/patio', { ...filters, pagina: page, tamanho_pagina: 100 });
      setData(result);
      setPagina(page);
