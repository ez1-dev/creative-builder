...
export default function ExpedidoObraPage() {
  ...
  const [localFilters, setLocalFilters] = useState({
    codigo_produto: '', numero_carga: '', data_ini: '', data_fim: '',
  });
  ...
  useAiFilters('producao-expedido', (updater) => {
    if (typeof updater === 'function') {
      const result = updater(filters);
      const { codigo_produto, numero_carga, data_ini, data_fim, ...shared } = result;
      setSharedFilters(shared as any);
      setLocalFilters({ codigo_produto, numero_carga, data_ini, data_fim });
    }
  }, () => search(1));
  const clearFilters = () => {
    clearSharedFilters();
  ...
  return (
    <div className="space-y-4 p-4">
      ...
      <FilterPanel onSearch={() => search(1)} onClear={clearFilters} onClearResults={clearResults}>
        ...
        <div><Label className="text-xs">Nº Carga</Label><Input value={localFilters.numero_carga} onChange={(e) => setLocalFilters(f => ({ ...f, numero_carga: e.target.value }))} className="h-8 text-xs" /></div>
