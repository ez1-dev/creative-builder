...
const FROTA_VISUALS = VISUAL_CATALOG.find((g) => g.module === 'Manutenção de Frota')?.items ?? [];
...
export function FrotaShareLinksDialog({ open, onOpenChange }: Props) {
  ...
  const [visiveis, setVisiveis] = useState<Record<string, boolean>>(() =>
    Object.fromEntries(FROTA_VISUALS.map((v) => [v.key, true])),
  );
  ...
  const load = async () => {
    ...
    const { data, error } = await supabase
      .from('manutencao_frota_share_links')
      .select('*')
  ...
  const handleCreate = async () => {
    ...
    const hiddenVisuals = FROTA_VISUALS.filter((v) => !visiveis[v.key]).map((v) => v.key);

    const { error } = await supabase.rpc('create_frota_share_link', {
    ...
    const link = `${baseUrl}/frota/compartilhado?token=${linkParam}${password ? '&p=1' : ''}`;
    setNovoLink(link);
    setNovoLinkProtegido(!!password);
    setNome(''); setSenha(''); setValidade('30');
    setVisiveis(Object.fromEntries(FROTA_VISUALS.map((v) => [v.key, true])));
    load();
    setCreating(false);
  };
  ...
  const copyOpenLink = (token: string) => {
    const link = `${baseUrl}/frota/compartilhado?token=${token}`;
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <LinkIcon className="h-5 w-5" /> Compartilhar Manutenção de Frota
          </DialogTitle>
        </DialogHeader>
