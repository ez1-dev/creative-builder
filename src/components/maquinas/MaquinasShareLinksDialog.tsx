...
const MAQUINAS_VISUALS = VISUAL_CATALOG.find((g) => g.module === 'Manutenção de Máquinas')?.items ?? [];
...
export function MaquinasShareLinksDialog({ open, onOpenChange }: Props) {
  ...
  const [visiveis, setVisiveis] = useState<Record<string, boolean>>(() =>
    Object.fromEntries(MAQUINAS_VISUALS.map((v) => [v.key, true])),
  );
  ...
  const load = async () => {
    ...
    const { data, error } = await (supabase as any)
      .from('manutencao_maquinas_share_links')
      .select('*')
  ...
  const handleCreate = async () => {
    ...
    const expiresAt = validade !== '0'
      ...
      : null;
    const hiddenVisuals = MAQUINAS_VISUALS.filter((v) => !visiveis[v.key]).map((v) => v.key);
    const { error } = await (supabase.rpc as any)('create_maquinas_share_link', {
      _token: effectiveToken, _nome: nome,
      _password: password ? 'protected' : null,
      _expires_at: expiresAt, _hidden_visuals: hiddenVisuals,
    });
    if (error) {
    ...
    const link = `${baseUrl}/manutencao-maquinas/compartilhado?token=${linkParam}${password ? '&p=1' : ''}`;
    setNovoLink(link);
    setNovoLinkProtegido(!!password);
    setNome(''); setSenha(''); setValidade('30');
    setVisiveis(Object.fromEntries(MAQUINAS_VISUALS.map((v) => [v.key, true])));
    load();
    setCreating(false);
  };
  ...
  const handleRevoke = async (id: string) => {
    const { error } = await (supabase as any)
      .from('manutencao_maquinas_share_links').update({ active: false }).eq('id', id);
  ...
  const copyOpenLink = (token: string) => {
    const link = `${baseUrl}/manutencao-maquinas/compartilhado?token=${token}`;
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <LinkIcon className="h-5 w-5" /> Compartilhar Manutenção de Máquinas
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="rounded-md border p-3 space-y-3 bg-muted/30">
            ...
            {MAQUINAS_VISUALS.length > 0 && (
              <div className="rounded-md border bg-background p-3 space-y-2">
                <Label className="text-xs font-semibold">Gráficos visíveis no link</Label>
                <p className="text-xs text-muted-foreground">Desmarque o que NÃO deve aparecer no link público.</p>
                <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                  {MAQUINAS_VISUALS.map((v) => (
                    <label key={v.key} className="flex items-center gap-2 text-sm cursor-pointer">
                      <Checkbox
                        checked={visiveis[v.key] ?? true}
                        onCheckedChange={(c) => setVisiveis((prev) => ({ ...prev, [v.key]: c === true }))}
                      />
                      <span>{v.label}</span>
                    </label>
                  ))}
          ...
          <div>
                  ...
                  {loading ? (
                    ...
                    return (
                      <TableRow key={l.id}>
                        ...
                        <TableCell className="text-xs">{hiddenCount === 0 ? 'Todos' : `${hiddenCount} oculto${hiddenCount > 1 ? 's' : ''}`}</TableCell>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Links protegidos por senha só podem ser copiados no momento da criação.
            </p>
          </div>
        </div>
